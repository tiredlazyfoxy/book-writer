...
`/architect` gate. The one part still undesigned is the **FEAT-013 assistant
subsystem** (agent loop, tool/MCP protocol, SSE shared-canvas writes, web search,
model selection); its design is resolved during planning of the features that
touch it (`011`, `013`, `018`, `019`, and the drafting half of `016`) and
reconciled back to `docs/architecture/` after.
...
| Feature | Track | Size | Delivers | Depends on | Definition |
|---|---|---|---|---|---|
| `008.data-domain` | multi-step | L | — | `007.database-consistency` | Persist every book-domain entity: one db module + JSONL codec per table; register codex as first vector source. |
...
| `011.chat-panel` | multi-step | L | FEAT-013 (chat + assistant + web) | `010.working-page` | Live chat pane: create/list/continue/archive chats; converse with the assistant; web search. |
| `012.system-prompts-editor` | multi-step | M | FEAT-019 (placeholder) | `010.working-page`, `009.books` | Placeholder: edit a book's and a chapter's system prompt; spec/architecture to follow. |

## Stage 4 — Codex (briefed)

| Feature | Track | Size | Delivers | Depends on | Definition |
|---|---|---|---|---|---|
| `013.codex` | multi-step | L | FEAT-017 (core), FEAT-018 | `010.working-page`, `011.chat-panel` | Author/edit/browse/search codex entries; incremental embedding; assistant reaches and writes codex from chat. |
...
placed before `013.codex` per the confirmed plan. Numbers 008–019 (and
`fast/002`) are **allocated**, not indicative.
...
checks · UC-025 admin ownership reassignment (unless folded into `009.books`).
...
- `011.chat-panel`, `013.codex`, `018.chapter-history-variants`,
  ...
  **undesigned FEAT-013 assistant subsystem** — context assembly, the
  tool/function-call protocol, the agent loop, the SSE shared-canvas event
  protocol, model selection, web search — as a planner open question; resolved
  during planning and reconciled to `docs/architecture/` after. No architect
  gate is placed on this.
