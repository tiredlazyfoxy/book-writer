# 020.assistant-config-editor — Assistant configuration (admin)
<!-- roadmap:start -->
- **Stage:** 3.workspace · **Track:** multi-step · **Size:** L
- **Delivers:** FEAT-020, UC-095, UC-096, UC-097, US-110, US-111, US-112, US-113, US-114
...
configured server+model, or inherit the main chat's model), and its own
tool selection. Wire modes and sub-agents together from either the mode
view or the sub-agent view — one link, two editors.
...
**Out:** the runtime that consumes this config (`013`); the tool
implementations (registry is code; tools land with their owning features);
author book/chapter prompts (`012`); main-chat model selection (deferred
FEAT-013).
