...
- `INFO` for incoming API requests and key lifecycle events; `DEBUG` for full flow (LLM request/response, tool calls, internal results). The level policy is independent of which sinks are attached.

Since `fast/006.dev-file-logging` there is a second, **optional** sink for development. Its semantics are non-obvious, so each choice is recorded with its reason — a bare statement of the behaviour would read as an oversight to someone who does not know why it was made.

- **Setup lives in `app/logging_config.py`, not `app/main.py`.** The module exposes a single `configure_logging(...)` entry point, called once from the composition root. It is a cross-cutting top-level module outside the `routes`/`services`/`db`/`models` layering, following the existing `app/settings.py` / `app/ids.py` precedent — logging configuration belongs to no layer and is needed before any of them exist.
- **The console handler is the only always-on sink.** The third-party quieting (`aiosqlite`, `httpx`, `httpcore` → `WARNING`) is unchanged in substance; it merely moved inside that call.
- **The file sink is off unless `BOOKWRITER_LOG_DIR` names a directory.** Two settings drive it: `BOOKWRITER_LOG_DIR` (`Path | None`, default `None`) and `BOOKWRITER_LOG_BACKUP_COUNT` (`int`, default `10`, constrained `ge=0`). Only `start.ps1 -app` sets the directory, at a git-ignored `logs/` at the repo root. That default-off gating is the reason — not a coincidence — that the pytest suite and the containers stay console-only: because nothing else sets the variable, no change was needed to `backend/tests/conftest.py`, the `Dockerfile` or `docker-compose.dev.yml`. Anything that wants the file sink opts in by setting the variable; anything that does not, is unaffected by construction.
- **The file rotates on process start, not on size.** One fixed filename `bookwriter.log`, rolled exactly once at startup via a `RotatingFileHandler` with `maxBytes=0` (size-based rotation disabled, so the handler never rolls itself mid-run) plus an explicit `doRollover()`. The previous run becomes `.log.1` and older runs shift down to the backup-count limit. A stable filename was chosen over per-run timestamped files so that tailing and grepping have a fixed target, and because the stdlib handler already implements both the renaming and the pruning — a timestamped scheme would mean writing and maintaining both. Note that `backup_count=0` passes the `ge=0` constraint but degenerates: the stdlib handler then keeps no numbered backups and **appends to the live file instead of truncating it**, so `bookwriter.log` grows without bound across runs — use `>= 1`. A rollover `OSError` (typically a file locked by another process on Windows) is swallowed with a console warning: logging must never block boot.
- **`uvicorn` and `uvicorn.error` are attached to the file handler by name.** uvicorn sets `propagate = False` on its own loggers, so attaching to the root logger alone would silently drop the startup, shutdown and traceback lines — exactly the lines a dev log file exists for. **`uvicorn.access` is deliberately excluded**, because per-request lines would swamp the file.
- **One shared handler instance is attached to all three loggers** (root, `uvicorn`, `uvicorn.error`), and the idempotency marker is per-handler, so a repeat `configure_logging` call detaches it from all three and closes it once. This is written down because the natural-looking alternative is wrong: attaching a separate file handler per logger would double every uvicorn line in the file.
- **Accepted consequence under `--reload`:** the reloader child re-imports the app on every code change, so each reload rolls the file and the backups turn over quickly while editing. `BOOKWRITER_LOG_BACKUP_COUNT` raises the depth if more history is needed.
...
- **2026-08-09 — Opt-in development log file (`fast/006.dev-file-logging`).** Logging setup moved into `app/logging_config.py` and gained a second sink. Two choices are worth recording. (1) **Rotate at process start on a fixed filename** (`bookwriter.log`, `maxBytes=0` + explicit `doRollover()`) rather than per-run timestamped files — a fixed target for tail/grep, the stdlib handler already does the renaming and the pruning, and the backup count bounds disk use. (2) **Default-off env gating on `BOOKWRITER_LOG_DIR`** rather than an always-on file sink — one knob decides, and because only `start.ps1 -app` sets it, `conftest.py`, the `Dockerfile` and the compose stacks needed no change at all. `uvicorn.access` is excluded from the file on purpose. See "Logging" for the detail.
