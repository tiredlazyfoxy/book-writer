...
**UC-047, US-049 and UC-080 are cited although `docs/product/` records them `deferred`.** The design realizes them and the code shipped; what has not happened is a *run*. The five close-chapter tools ship registered but unreachable until an administrator assigns them to the `close-chapter` mode (`assistant-runtime.md`), so "deferred" here means *built, not yet exercisable* — not *not designed*.
