...
Part of the book-domain model. **Index and cross-cutting conventions: `domain-model.md`.** Related: `retrieval.md` (`CodexEntry` is the first vector-backed model), `domain-continuity.md` (a state note names an entry), `authorization.md` (the codex is members-only), `frontend-workspace.md` (the navigator's Characters / Locations / Facts entries).
...
`CodexEntryVersion` is drawn now but first *built* at Stage 5 (`018.codex-history-copy`), per the whole-map reasoning in `domain-model.md`.
...
Everything about *how* the assistant produces that draft is out of scope for this pass — see `domain-chat.md`.
