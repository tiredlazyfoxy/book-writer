...
**The transcript follows the newest message (`fast/010.transcript-autoscroll`).** It **follows the bottom while content arrives when the author is within one line of it — 64 CSS pixels**. That threshold was chosen over the two obvious alternatives because exact-bottom unpins on routine subpixel and zoom error (the follow would look broken for no visible reason) and a generous ~200px would yank down an author who had deliberately scrolled back a little. When the author is further up, **the position is preserved silently** and **nothing is rendered to announce the new content** — no pill, badge, button or count, deliberately. The transcript **always starts at the bottom** on mount, reload, chat switch, **send and retry**. Two of those are decisions rather than accidents. **Send and retry re-pin** even from a scrolled-up position, because asking for output is an explicit "done reading back" gesture and an author who sends expects to watch their own message land and the reply arrive under it; retry is the same gesture from the same composer, so treating it differently would be an inconsistency nobody could explain. And **nothing is persisted** — no `localStorage`, no URL param, no `workspaceLayout.ts` field — which is precisely what "always to bottom on reload" means.

**Where the follow lives, and why it is not in the leaf.** It is a **second `autorun` inside `WorkspaceShell`'s existing mount effect**, disposed in the existing cleanup beside the `--work-chat-width` disposer — not an effect in `MessageList`, which still holds **none**. The leaf contributes exactly two things: it hands the `ScrollArea`'s **viewport** element to `ChatPaneState` through `viewportRef` — a **callback ref, which is a plain prop and not a hook** — and reports scroll position through `onScrollPositionChange`. **The `ScrollArea`'s own `ref` targets the root, non-scrolling wrapper**, so `viewportRef` is the only handle on the element that actually scrolls; that is the single most likely wrong turn for the next person who touches this pane, and it costs one sentence to prevent. The general rule and its mechanics are `frontend.md` → React hook rules; this is where they are built.
...
**The transcript's pinned flag, viewport element and pending-frame handle are non-observable on purpose (`fast/010`).** All three are slots in `ChatPaneState`'s `makeAutoObservable` exclusion map, beside `fast/008`'s `composerResizeDispose`. Two reasons, the second load-bearing: the pinned flag is rewritten on **every scroll event, at pointer rate**, so an observable flag would fire the pane's observers continuously while the author merely scrolls; and the follow `autorun` **reads** it, so an observable flag would make the autorun **re-enter itself on its own programmatic scroll**. Related and equally deliberate: **the programmatic scroll's own scroll event is not suppressed.** Writing `scrollTop` re-runs the scroll handler, which — because the write landed at the bottom — recomputes to pinned-true and terminates on its first iteration. A suppression flag ("ignore the next scroll event") would also swallow a genuine user scroll landing in the same frame, which is the one event that must never be missed. Both of these read as omissions to a newcomer — a missed annotation and an unclosed loop — and both are decisions with a failure mode behind them.
