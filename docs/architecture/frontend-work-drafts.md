...
**Memos are outside the buffer too (FEAT-021).** The memos list is the content pane's first **editable list** (`frontend-workspace.md`), so it is the first editable region that has to be excluded explicitly rather than inherited from "any list is read-only". The reasoning is the per-author prompt's, unchanged: the buffer exists for **large content-pane artifacts carrying a version token** whose loss is expensive, while a memo is **short**, saved **on focus loss within moments of being typed** (UC-104), and has exactly **one writer — its owner**, who is also its only reader. Same consequences as the other exclusions: **no `baseVersion`, no stale-buffer detection, no divergence view, no `409` path** — and nothing to compare a `baseVersion` against, since a memo carries no version token at all (`domain-book.md`).
