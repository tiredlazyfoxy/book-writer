...
| Work | `/work/:bookId/chats` | The book's **chats list** in the content pane; a picked row opens in the chat pane | **yes** (011, `023`) |
...
- **`/work/:bookId/chats` is an ordinary content-pane route rendering the book's chats list, and the Chats navigator entry is an ordinary router link** (feature `023.chat-ux-revision`, reversing feature 011). A **picked** chat opens in the chat pane through a module-level controller seam, with no route change. `docs/product/` was reconciled to match on 2026-08-10; the decision history — including why 011 resolved it the other way — is in `frontend-workspace.md` → the route map.
...
4. **Seven named frames** reach the wire — **`thinking`**, **`delta`**, **`done`**, **`error`** (feature 011), **`canvas`** (feature 013, the shared-canvas codex draft) and **`tool_call`** / **`tool_result`** (feature `024`, tool-call visibility). The route serializer is `event: <name>` / `data: <payload JSON>` and is **generic over the event name**, which is why each frame added since the first four needed no route change. A failure *after* the stream opens is an `error` frame over HTTP 200 — never a 500, never a hung stream. Payload shapes are in `quick-reference.md`; the protocol and its reasoning are in **`assistant-runtime.md`**.
...
**Deployment requirement:** assistant *thinking* needs one of **two working server shapes** — llama.cpp running with **`--reasoning-format none`** (reasoning inlined into `content`), or a server sending structured `reasoning_content` against **`llm-client v0.1.5` or later**, which re-emits it wrapped in `<think>` / `</think>`. Below `v0.1.5` the second shape degrades **silently** to content-only. The detail is inside the `llm-client` dependency — see `backend/features.md` → "Deployment requirement".
