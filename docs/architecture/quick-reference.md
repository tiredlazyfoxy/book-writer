...
### `/api/books/{book_id}/memos` (FEAT-021, **designed**) — every route `Depends(authz.book_access)`

| Method | Path | Success | Body → Response | Notes |
|---|---|---|---|---|
| `GET` | `/api/books/{book_id}/memos` | `200` | `?include_archived=false` → `MemoListResponse` | the **caller's own** memos only; ordinal-ordered |
| `POST` | `/api/books/{book_id}/memos` | `201` | `CreateMemoRequest` → `MemoResponse` | server-assigned ordinal — **appended last**; `body` may be `""` (UC-103) |
| `PUT` | `/api/books/{book_id}/memos/order` | `200` | `ReorderMemosRequest` → `MemoListResponse` | the **full** id list of the caller's non-archived memos, validated whole before any write, then ordinals rewritten `1..N`. Static segment declared **before** the `/{memo_id}` routes |
| `PUT` | `/api/books/{book_id}/memos/{memo_id}` | `200` | `UpdateMemoRequest` → `MemoResponse` | **body only** — the focus-loss save (UC-104). **No version token** |
| `POST` | `/api/books/{book_id}/memos/{memo_id}/activate` | `200` | **no body** → `MemoResponse` | the on/off axis (UC-106) |
| `POST` | `/api/books/{book_id}/memos/{memo_id}/deactivate` | `200` | **no body** → `MemoResponse` | |
| `POST` | `/api/books/{book_id}/memos/{memo_id}/archive` | `200` | **no body** → `MemoResponse` | the archive axis (UC-107) |
| `POST` | `/api/books/{book_id}/memos/{memo_id}/restore` | `200` | **no body** → `MemoResponse` | returns to the working list, **appended last** |

- **Status taxonomy:** **401** no token; **404** a private book the caller has no relationship to (from `resolve_book_access`, **not re-derived**); **403** a logged-in non-member of a book they can see; **404** for a memo that does not exist; **200** (`201` on create) for a member acting on their own memo. **Another author's memo and another book's memo are the same 404 as an unknown id** — one refusal, no existence oracle. **No `Capability` member and no `_CAPABILITY_MATRIX` row**; it is the **fourth row-ownership rule** (`authorization.md`).
- **An archived book does NOT refuse these writes** — the one named carve-out from the book-state gate (`authorization.md`). Quarantined and destroyed are unchanged.
- **Why two route pairs rather than one `PATCH`:** the on/off switch and the archive axis are **two independent axes in the model** (`domain-book.md`), and giving each its own verb pair makes that visible on the wire while keeping the focus-loss `PUT` from carrying state it did not mean to write. Mirrors `SubAgent`'s `disable` / `enable` precedent.
- **Deliberate absences:** **no `DELETE`** (archive-not-delete, UC-107/US-128.AC-3); **no per-memo move endpoint** (one bulk `PUT`, the `chapters/order` reasoning — concurrent callers cannot interleave into an ordinal set nobody chose); **no memo field on any book DTO** (a per-caller value cannot ride on a book-shaped DTO — the `BookAuthorPrompt` reasoning); **no `expected_modified_at` and no `409`** (one writer per row, `domain-book.md`).
- **`MemoResponse` carries no `user_id`** — the subject is always the caller, and returning an id would invite the reading that another author's memo is addressable here. It is not. Same reasoning as `BookAuthorPromptResponse`.
...
| `SubjectKind` | module-level `Literal` — `book-state` \| `chapters` \| `chapter` \| `characters` \| `locations` \| `facts` \| `codex-entry` \| `variants` \| `chapter-variants` \| `chats` \| **`memos`** (FEAT-021); value-for-value with `frontend/src/work/subject.ts`. `memos` is **mode-less**, like `chapters` / `variants` / `chats` — it changes no routing (tool-gating case 2 either way) and **widens neither `ResolvedSubject` nor `determine_mode`**; see `assistant-runtime.md` |
...
### Memos (FEAT-021, **designed**) — `app/models/schemas/memos.py`

| DTO | Shape |
|-----|-------|
| `CreateMemoRequest` | `body: str` — no constraint; **`""` is legitimate input** (a new memo is created empty and focused, UC-103). **No `ordinal`** — a new memo is appended |
| `UpdateMemoRequest` | `body: str` — **body only**, and **no version token**: a memo is last-write-wins with exactly one writer, so there is no `expected_modified_at` and no `409` |
| `ReorderMemosRequest` | `memo_ids: list[str]` — the **full** ordered set of the caller's non-archived memos; a list that is not exactly that set is refused **400** (the `ReorderChaptersRequest` precedent) |
| `MemoResponse` | `id: str`, `book_id: str`, `body: str`, `ordinal: int`, `active: bool`, `archived: bool`, `created_at`, `modified_at`. **No `user_id`** — the subject is always the caller (the `BookAuthorPromptResponse` / `ChapterAuthorPromptResponse` rule) |
| `MemoListResponse` | `items: list[MemoResponse]` |
...
| `Memo` | `app/models/memo.py` (`db/memos.py`) | **FEAT-021, designed.** Surrogate **snowflake PK**, `book_id` FK → `books.id`, `user_id` FK → `users.id` (the author; **nobody else ever reads the row**, including the book's owner), `body` (**required**; `""` is legal — a new memo is created empty and focused), `ordinal` (non-null int, scoped **per `(book, author)`**), `active` (bool, default `true`), `archived` (bool, default `false`), `created_at` / `modified_at`. **Two booleans, not one state enum** — the deliberate opposite of `Book.state`: inactive and archived are two axes, and only two flags remember whether a restored memo was on or off before archiving. **No unique `(book_id, user_id)` constraint** — unlike `BookAuthorPrompt` / `BookMember`, an author has **many** memos per book; this is the one thing a reader will get wrong from the field list. **Ordinal rules:** new → `max+1`; archive **leaves a gap, no renumber** (the `chapters` DELETE precedent); **restore appends last**, not to its old slot — so the list read needs **no `(ordinal, id)` tiebreak**. Context membership is **derived**: reaches the assistant iff `archived == false AND active == true`. **No version token, no `409`.** `TABLE_REGISTRY` index **11**, immediately after `chapter_author_prompts` — FK-correct **and** adjacent to the other per-author tables, so **not** a second exception to FK ordering. **No `VECTOR_SOURCE_REGISTRY` entry** — every active memo is pushed whole into every turn, so there is nothing to retrieve. See `domain-book.md`, `authorization.md`, `assistant-runtime.md`. |
