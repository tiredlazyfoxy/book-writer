...
| `TOOL_REGISTRY` | code (e.g. `services/assistant/tools.py`) | **Not a table** — module-level `list[ToolDef]` (`name`, `description`, `args_schema: type[BaseModel]`, `callable`), mirroring `TABLE_REGISTRY` / `VECTOR_SOURCE_REGISTRY`. Selections persist by string `name`; the catalogue is **never exported**. See `assistant-config.md`. |
