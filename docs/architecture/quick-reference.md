...
| `POST` | `/api/books/{book_id}/chapters/{chapter_id}/close` | `200` | **no body** → `ChapterResponse` | `open → closed` **directly** at Stage 2 — the ungated close; `016` re-points it at `closing`. **Owner-only** |
...
- **Four chapter tools** joined `TOOL_REGISTRY` (`services/chapter_tools.py`) — read the saved body, replace the whole body, replace the selection, append to end — **mode-gated and shipped unreachable** until an admin selects them for `write-chapter`. See `assistant-runtime.md`.
...
| `ChapterResponse` | `id: str`, `book_id: str`, `ordinal: int`, `title: str`, `state: ChapterState`, `sketch: str`, `version: int`, `created_at`, `modified_at`. **No `text`, no `summary`, no `summary_status`, no `system_prompt`** — the body is `015`'s, the summary `016`'s, and the prompt has its own route |
...
- **`src/work/`'s module tier has four members:** `restoreBuffer.ts` (010), `activeChat.ts` (011), `contentSubject.ts` (013 — canvas targets, plus 015's selection registry), **`chapterUndo.ts` (015 — assistant-write undo snapshots per `(book, chapter)`, capped at 20, in memory only)**. Full sanctions in `frontend-work-drafts.md`.
