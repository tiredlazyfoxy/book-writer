...
- **One open, now covering both halves of FEAT-019.** The fifth — feature `021.per-author-system-prompt` replaced FEAT-019's **book-wide, owner-only** system prompt with a **per-author, per-book** one (`BookAuthorPrompt`), and feature `014.chapter-skeleton` did the same one level down with `ChapterAuthorPrompt`, superseding **UC-094 and every criterion of US-109** — is **OPEN, awaiting `/product-spec`**. US-108.AC-2 is reversed outright rather than superseded, and no product id is cited for either new entity, deliberately: citing UC-093 / US-108 / UC-094 / US-109 would claim satisfaction of criteria this design contradicts. `/product-spec` must rewrite FEAT-019 **whole**; the ids it mints are what a later feature will cite.
