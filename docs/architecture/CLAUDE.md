...
- **`assistant-runtime.md` — the runtime that consumes it.** Mode determination and the `TurnRequest` wire shape, the **five-layer** system-prompt composition (four until FEAT-021 inserted the memos layer at position 4), the three-case tool gating, the `chat_with_tools` protocol, sub-agent delegation, **per-chat model selection**, **web search**, the **seven-frame** SSE vocabulary (`thinking` / `delta` / `done` / `error` / `canvas` / `tool_call` / `tool_result`), the **shared-canvas write protocol for codex entries and for chapters**, and — since feature `016` — the **close-chapter tool set and the post-turn deterministic finalize hook** (a turn is no longer "stream, persist, done", though the frame vocabulary is unchanged).

**Also covered, since 2026-09-15 — FEAT-021 (memos)**, designed as a round of its own and not yet carried by a plan folder: the **`Memo` entity** (`domain-book.md`, plus the clone rule US-133), authorization's **fourth row-ownership rule** and its named archived-book carve-out (`authorization.md`, plus US-134's moderation exclusion), the **composed `MEMOS` prompt layer** and its threading into sub-agent delegation, the **`create_memo` tool** and the widened `BASE_TOOL_NAMES` (`assistant-runtime.md`, `assistant-config.md`), and the **Memos navigator entry and `/work/:bookId/memos` route** (`frontend-workspace.md`).

**Still not covered — do not infer it:** **context / content assembly** (US-057, UC-085/086/078 internals), **token-level canvas streaming**, and **token budgeting**. A working chat pane and a working canvas write invite the reading that the context model shipped with them; it did not. **FEAT-021 does not change this, and is the likeliest thing to be misread as changing it:** memos are **authored** material entering the prompt as a **named layer**, not **retrieved** material the system selected — no retrieval, no ranking, no relevance question, no truncation. The covered / not-covered pair above is internally consistent on exactly that distinction. See `domain-chat.md` for the boundary and `assistant-runtime.md` → "Out of scope" for the same list with reasoning.
